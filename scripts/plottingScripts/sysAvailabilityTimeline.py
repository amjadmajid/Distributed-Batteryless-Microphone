...
def main():
    fontSize=16
    ontime_path = '../processed_data/availabilityTimeline470.json'
    ...
    plt.xlabel("light intensities", fontsize=fontSize)
    plt.xticks(np.arange(5)+1,(300,500,800,1000,1400),fontsize=fontSize-2)
    plt.yticks(fontsize=fontSize-2)
    plt.xticks(fontsize=fontSize-2)
    plt.tight_layout()
    plt.savefig('../../paper/figures/sysAvailabilityTimeline.eps')
    plt.show()
