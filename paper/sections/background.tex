...
they also limit sensors' lifetimes~\cite{aditya2008comprison}.  
%
...
This guarantee facilitates the software architecture design. An intermittent program is usually decomposed  into a chain of small atomic regions that  are  glued together with checkpoints~\cite{mementos}. 
...
the size requirements of an application. For example, the WISPCam---an RF powered camera---~\cite{naderiparizi_rfid_2015} uses a supercapacitor that is about 50 times the size of the original WISP's capacitor---a computational RFID tag---~\cite{smith_ubicomp_2006}.
