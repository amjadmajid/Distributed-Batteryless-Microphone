...
\begin{figure}[t]
		\centering
% 		\includegraphics[width=\columnwidth]{fff}
		\caption{\todo{To be done}\todo{Show the correlation, between the off-time and the on-time. Experiment setup: (1) one node, (2) vary light intensity from 300 to 1500 every 10 second}. Conclusion, if you observe an (inverse) correlation between the on-time and the off-time, then we validate the timing algorithm}
		\label{fig:solarPwrCoIS}
\end{figure} 
