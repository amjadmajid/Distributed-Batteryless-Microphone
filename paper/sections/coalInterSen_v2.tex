...
$= \min \left(N\times \frac{\overline{t_\text{on}}}{\overline{t_\text{p}}}\bigg\rvert 100\%\right)$. However, we expect such an approach to introduce significant overhead as a scattering algorithm (e.g., \cite{giusti2007decentralized}) must be frequently executed, messages need to be exchanged, and clocks should be synchronized. Therefore, we propose a different on-times spreading strategy.  
%
\begin{figure}[t]
		\centering
...
\begin{figure}
		...
		\caption{\fullcis availability percentage for a different number of nodes and different duty cycles. The nodes are uniformly distributed and the \cis on-time evolves, when adding new nodes, according to the equation~\ref{eq:cisModel}.}
		\label{fig:cisModel}
\end{figure} 
%
...
where $t_\text{s}$ is the expected sleep time of the \cis's nodes, 
 ...
 and $e_\text{a}$ and $e_\text{s}$ are the energy consumption at active and sleeping modes, respectively.
%
...
\begin{figure}[t]
		...
		\begin{subfigure}{.49\columnwidth}
			...
			\caption{RF}
		\end{subfigure}\hfill
		\caption{The average duty cycles of 8 solar-powered and 6 RF-powered intermittent nodes for different ambient energy sources and energy intensities. In general, the average duty cycle of a node is a good indicator of the average duty cycle of \cis's nodes.}
...
\begin{enumerate*}[label=(\roman*)]
 \item the total number of nodes in its \cis, which is a constant value that can be loaded to the device memory; 
  \item the on-times distribution, which is uniform in our case; and
 \item its own $\overline{t_\text{on}}$ and $\overline{t_\text{off}}$. 
\end{enumerate*}

Since, we assume that a \cis's nodes have the same energy buffer and are in the vicinity of each other---thus, they are exposed to the same energy conditions---then their duty cycles should approach the same value. 
Figure~\ref{fig:cis_nodes_dutyCycle} shows the average power cycles of the nodes of a solar- and RF-powered \cis{}s. In general, we can conclude that a node's average duty cycle is a good estimator of other \cis's nodes' duty cycles.
