...
def generate_node(num_pwr_cycles, on_time, off_time):
	power_up = np.random.uniform(0,num_pwr_cycles ,1)
	return (power_up, on_time, off_time)
...
def time_span(nodes):
    ...
    while idx < num_nodes-1:
    	if (sorted_nodes[idx][0]+sorted_nodes[idx][1]) > sorted_nodes[idx+1][0]+(sorted_nodes[idx+1][1]):
    		del sorted_nodes[idx+1] # do not advance the counter because you delete the node
    	else:
    		idx+=1

    for i in range(num_nodes-1):
        dif = sorted_nodes[i+1][0] - sorted_nodes[i][0]
        node_on_time = sorted_nodes[i][1]
        if dif > node_on_time:
        	span+=node_on_time
        else:
            span+=dif
